...
class Database:
    ...
    async def change_balance(self, user_id: int, amount: float) -> None:
        await self.execute("UPDATE users SET balance = balance + $2 WHERE user_id = $1", user_id, amount)
    ...
    async def update_category_name(self, category_id: int, new_name: str) -> None:
        await self.execute("UPDATE categories SET name = $2 WHERE id = $1", category_id, new_name)

    async def category_has_products(self, category_id: int) -> bool:
        count = await self.fetchval("SELECT COUNT(*) FROM products WHERE category_id = $1", category_id)
    ...
    async def update_product_name(self, product_id: int, new_name: str) -> None:
        await self.execute("UPDATE products SET name = $2 WHERE id = $1", product_id, new_name)
    ...
    async def update_product_photo(self, product_id: int, photo_file_id: str | None) -> None:
        await self.execute("UPDATE products SET photo_file_id = $2 WHERE id = $1", product_id, photo_file_id)

    async def update_product_category(self, product_id: int, category_id: int) -> None:
        await self.execute("UPDATE products SET category_id = $2 WHERE id = $1", product_id, category_id)
    ...
    async def remove_cart_item(self, product_id: int, user_id: int) -> None:
        await self.execute("DELETE FROM cart_items WHERE product_id = $1 AND user_id = $2", product_id, user_id)
    ...
    async def create_order_from_cart(
        ...
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                await conn.execute("UPDATE users SET balance = balance - $2 WHERE user_id = $1", user_id, total_amount)
    ...
    async def get_user_orders(self, user_id: int):
        return await self.fetch(
            "SELECT id, total_amount, address, phone, latitude, longitude, status, created_at FROM orders WHERE user_id = $1 ORDER BY id DESC",
            user_id,
        )

    async def get_all_orders(self):
        return await self.fetch(
            "SELECT id, user_id, total_amount, address, phone, latitude, longitude, status, created_at FROM orders ORDER BY id DESC LIMIT 50"
        )

    async def create_support_ticket(self, user_id: int, username: str | None, full_name: str, message: str) -> int:
        return int(
            await self.fetchval(
                "INSERT INTO support_tickets (user_id, username, full_name, message) VALUES ($1, $2, $3, $4) RETURNING id",
                user_id,
                username,
                full_name,
                message,
