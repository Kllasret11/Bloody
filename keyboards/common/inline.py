...
def categories_kb(categories) -> InlineKeyboardMarkup:
    ...
    for category in categories:
        kb.add(InlineKeyboardButton(text=category.get("name", "Категория"), callback_data=f"cat:{category.get('id')}"))
    return kb
...
def product_item_kb(product_id: int, qty: int = 1) -> InlineKeyboardMarkup:
    ...
    kb.add(InlineKeyboardButton("🛒 Добавить в корзину", callback_data=f"addcart:{product_id}:{qty}"))
    return kb


def cart_item_kb(product_id: int) -> InlineKeyboardMarkup:
    kb = InlineKeyboardMarkup(row_width=1)
    kb.add(InlineKeyboardButton("❌ Удалить", callback_data=f"cartdel:{product_id}"))
    return kb
...
def checkout_kb() -> InlineKeyboardMarkup:
    ...
    kb.add(InlineKeyboardButton("🏷 Ввести промокод", callback_data="promo"))
    return kb


def reorder_kb(order_id: int) -> InlineKeyboardMarkup:
    kb = InlineKeyboardMarkup(row_width=1)
    kb.add(InlineKeyboardButton("🔁 Повторить заказ", callback_data=f"reorder:{int(order_id)}"))
    return kb
